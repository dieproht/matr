...
/** Default determinant implementation based on Laplace expansion. Please pay attention: its
  * algorithmic simplicity comes with a high perfomance cost!
  */
trait DefaultDeterminant:
   ...
   given defaultDeterminant[R <: Int, C <: Int, T]
      ...
      private def excludeRowAndCol
               (excludeRow: Int, excludeCol: Int, ref: (Int, Int) => T)
               : (Int, Int) => T =
         (rowIdx: Int, colIdx: Int) =>
            val rowIdxN =
               if rowIdx >= excludeRow then
                  rowIdx + 1
               else
                  rowIdx
            val colIdxN =
               if colIdx >= excludeCol then
                  colIdx + 1
               else
                  colIdx
            ref(rowIdxN, colIdxN)
