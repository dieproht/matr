...
trait DefaultMatrixMultiplication:
   ...
   given defaultMatrixMultiplication[R <: Int, C <: Int, L <: Int, T, U, X]
            (using MatrixFactory[R, L, X], Numeric[X], ElementMultiplication.Aux[T, U, X])
            (using ValueOf[R], ValueOf[C], ValueOf[L])
            : MatrixMultiplication[R, C, L, T, U] with

      def dot(lhs: Lhs, rhs: Rhs): Out =
         val numX = summon[Numeric[X]]
         val mul = summon[ElementMultiplication.Aux[T, U, X]]
         ...
         while j < lhs.colDim do
            ...
            while k < lhs.rowDim do
               ...
               while l < resultNumCols do
                  if j == 0 then buildr(k, l) = mul.times(lhs(k, j), rhs(j, l))
                  else buildr(k, l) = numX.plus(buildr(k, l), mul.times(lhs(k, j), rhs(j, l)))
                  l = l + 1
               k = k + 1
            j = j + 1
         buildr.result
