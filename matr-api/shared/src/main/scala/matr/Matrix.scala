...
/** Represents a matrix in the mathematical sense and is the central type of this library. Row dimension, column
  * dimension and the data type of the elements are set at compile-time. Elements are indexed zero-based. Concrete
  * subclasses of this trait implement immutable storage and access to the elements of the matrix. Implementations for
  * non-mathematical helper methods (`mkString` etc.) are provided here, but can be overridden. Mathematical operations
  * are handled by type classes.
  *
...
trait Matrix[R <: Int, C <: Int, T](using Matrix.Requirements.NonNegativeDimensions[R, C])(
    ...
    /** Performs a Matrix Multiplication by calculating the dot product.
      */
    final infix def dot[L <: Int, U, X](rhs: Matrix[C, L, U])(
    ...
    /** Folds over the elements of this Matrix by invoking the specified operation with each element's index and the
      * result of the previous invocation.
      *
      * @param start
...
object Matrix:
    ...
    /** A `Builder` is a "Matrix under construction" that itself forms a (mutable) Matrix. Utilizing `Builder` is the
      * preferred way of creating new Matrices in Matrix operations.
      *
      * Modules implementing the `Matrix` trait should also provide an implementation of this trait.
      *
      * Invoking the same `Builder` instance multiple times is OK, but letting a `Builder` leave the local scope is not
      * OK (it's mutable!).
      *
