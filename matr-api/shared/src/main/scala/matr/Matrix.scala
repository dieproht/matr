package matr:
   ...
   /** Represents a matrix in the mathematical sense and is the central type of this library. Row
     * dimension, column dimension and the data type of the elements are set at compile-time.
     * Elements are indexed zero-based. Concrete subclasses of this trait implement immutable
     * storage and access to the elements of the matrix. Implementations for non-mathematical helper
     * methods (`mkString` etc.) are provided here, but can be overridden. Mathematical operations
   ...
   trait Matrix[R <: Int, C <: Int, T]
      ...
      /** Allows to easily "modify" this Matrix by creating a `Builder` initialized with the values
        * of this Matrix.
        */
