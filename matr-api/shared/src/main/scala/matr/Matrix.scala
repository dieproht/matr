package matr:
  ...
  trait Matrix[R <: Int, C <: Int, T]
     ...
     * 
     * @tparam RowIdx row index
     * @tparam ColIdx column index
     */ 
    def apply[RowIdx <: Int, ColIdx <: Int]
             (using RowIdx < R =:= true, ColIdx < C =:= true)
             (using vri: ValueOf[RowIdx], vci: ValueOf[ColIdx])
             : T = 
      apply(vri.value, vci.value)
    ...
    */
     ...
     */ 
    final infix def dot[L <: Int, U](rhs: Matrix[C, L, U])
     ...
     * 
     * @tparam RowIdxTL row index of the top-left vertex
     * @tparam ColIdxTL column index of the top-left vertex
     * @tparam RowIdxBR row index of the bottom-right vertex
     * @tparam ColIdxBR column index of the bottom-right vertex
     */
     ...
     */ 
    def combine[U, V](rhs: Matrix[R, C, U])
                     (op: (T, U) => V)
     ...
     * 
     * @param start value provided to the first invocation 
     * @param op operation applied to each element (index)
     * @return result of the last invocation of the specified operation
     */ 
    def fold[S](start: S)(op: (S, Int, Int) => S): S =
      impl.Fold(this, start, op)
