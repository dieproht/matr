...
object M:
   ...
   def apply[R <: Int, C <: Int, T]
            ...
            : DslBuilder[0, -1, R, C, T] = DslBuilder(mf.builder)
...
case class DslBuilder[CurRow <: Int, CurCol <: Int, R <: Int, C <: Int, T] private[dsl]
   ...
   def ||(v: T)(using CurRow < R =:= true): DslBuilder[CurRow, 0, R, C, T] = DslBuilder(
      builder(vCurRow.value, 0) = v
   )
   ...
   def |
            ...
            : DslBuilder[CurRow, CurCol + 1, R, C, T] = DslBuilder(
      builder(vCurRow.value, vCurColSucc.value) = v
   )
